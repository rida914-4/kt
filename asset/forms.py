...
from models import Item, Positions, Custodian, StkCat, TmpPlInvoice, TmpPlInvoicedet, Users
...
class LoginForm(AuthenticationForm):
    class Meta:
        model = Users
        exclude = ['emp_id', 'auth_id','use_form','use_rep','logged','comp_id','year_id','date_from','date_to','chasis_no','pk_bill_no_from','pk_bill_no_to','usd','acct_id','voucher_type','post','sp_id','u_session_id']
        widgets = {
            'emp_name':  forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Username'}),
...
class TmpForm(ModelForm):
    ...
    voucher_id = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '', 'name': 'voucher_id',
            }), required=False)

    voucher_date = forms.DateField(widget=forms.TextInput(
        attrs={'tabindex': '-1', 'class': 'form-control'}), required=False)

    particulars = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': ''}), required=False)
...
class TmpFormDetForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'voucher': forms.TextInput(
                attrs={'class': 'form-control', 'placeholder': '', 'name': 'voucher',
                       'readonly': 'readonly'}),
            'particulars': forms.TextInput(
...
class CheckPostedForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(CheckPostedForm, self).__init__(*args, **kwargs)
        print str(posted).lower()
        if str(posted).lower() == 'y':
            self.fields['posted'].widget = forms.CheckboxInput(attrs={'required': False})
            self.fields['posted'].initial = True
            print self.fields['posted'].initial
        else:
            self.fields['posted'].widget = forms.CheckboxInput(attrs={'required': False})
            self.fields['posted'].initial = False
            print self.fields['posted'].initial
    ...
    class Meta:
        model = TmpPlInvoice
        exclude = ['net_amt', 'post_date', 'address', 'particulars', 'pos_code', 'acct', 'cust', 'voucher_id', 'voucher_date']
        labels = {
            'posted': 'Posted ',
        }
